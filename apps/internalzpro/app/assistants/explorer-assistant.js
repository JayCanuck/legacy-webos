function ExplorerAssistant(params) {
	...
	this.WRITABLE = ["/media/cryptofs/apps", "/media/internal", "/var"];
...
ExplorerAssistant.prototype.setup = function() {
	...
	//Application Menu with an About entry
	var restartItem = {
		label: $L("Restart"),
		items: [
	...
	/*this.cmdMenuModel = {
		items: [
			{},
			{icon:"favourite", command:'favourites'}
		]
	};
...
ExplorerAssistant.prototype.doSort = function(){
	...
	if(this.writableDir() && this.appAssist.settings.explorer.swipeDelete) {
...
ExplorerAssistant.prototype.loadLists = function(){
	...
	if(this.writableDir()) {
		...
		this.menuModel.items = this.menuItems1;
	} else {
		this.fileListModel = this.fileListNoSwipeModel;
		this.controller.get("listFilesSwipeDelete").hide();
		this.controller.get("listFiles").show();
		this.menuModel.items = this.menuItems2;
	}
	...
	this.filemgr.listDirs(
		...
		this.grabDirsSuccess.bind(this),
		function(err) {
			Error($L(err.errorText));
...
ExplorerAssistant.prototype.grabDirsSuccess = function(response) {
	if(response.items.length!=0) {
		this.controller.get("line-between-lists").innerHTML='<hr/>';
	}
	this.dirListModel.items = response.items;
	this.controller.modelChanged(this.dirListModel);
	//this.updateListRowSizeIfNeeded("listDir");

	//next load files list
	this.filemgr.listFiles(
		{
			path: this.currDir,
			sort: this.sort,
			ascending: this.ascending,
		...
		this.grabFilesSuccess.bind(this),
		function(err) {
			Error($L(err.errorText));
		}.bind(this)
	);
...
ExplorerAssistant.prototype.grabFilesSuccess = function(response) {
	if(response.items.length!=0) {
		this.controller.get("line-after-lists").innerHTML='<hr/>';
	}
	this.fileListModel.items = response.items;
	...
	if(this.writableDir() && this.appAssist.settings.explorer.swipeDelete) {
		//this.updateListRowSizeIfNeeded("listFilesSwipeDelete");
	} else {
		//this.updateListRowSizeIfNeeded("listFiles");
	}
...
ExplorerAssistant.prototype.handleCommand = function(event){
  	if (event.type == Mojo.Event.command) {
		...
		case 'favourites':
			this.controller.stageController.pushScene("favourites",
		...
		case Mojo.Menu.helpCmd:
			...
			break;
		case 'showAbout':
			this.filemgr.version(
				function(response) {
					this.controller.showAlertDialog({
						onChoose: Mojo.doNothing,
						title: "",
						message: "<center><div style=\"font-size:1.35em;" +
							"margin-bottom:10px\"><strong>"
							+ Mojo.Controller.appInfo.title + "</strong>&nbsp;"
							+ "&nbsp;&nbsp; " + Mojo.Controller.appInfo.version
							+ "</div><div style=\"font-size:1.2em;margin-bottom:10px\">"
							+ "<strong>FileMgr Service</strong>&nbsp;&nbsp;&nbsp; "
							+ response.version + "</div><div style=\"font-size:1.15em\">"
							+ "<em>" + $L("Freeware") + "</em><br/><a href=\"http://"
							+ "m.canuck-software.ca\"><img src=\"images/canuck.png\"/>"
							+ "</a><div style=\"font-size:0.8em\">&copy; Copyright "
							+ "2009-2010, Jason Robitaille</div></center>",
					...
					this.controller.showAlertDialog({
			   			onChoose: function(value) {
							if (value == 'yes') {
								...
								if(this.writableDir()
										&& this.appAssist.settings.explorer.swipeDelete) {
...
ExplorerAssistant.prototype.writableDir = function() {
	return this.appAssist.settings.explorer.masterMode ||
			(this.currDir.startsWith(this.BASEDIR) ||
			this.currDir.startsWith(this.BASEDIR2) ||
			this.currDir.startsWith(this.BASEDIR3));
};
...
ExplorerAssistant.prototype.info = function(isDir){
	...
	this.controller.stageController.pushScene('info', {item: this.currItem,
			writableDir:this.writableDir(), filemgr:this.filemgr, dir:this.currDir,
			isDir:isDir});
};
...
ExplorerAssistant.prototype.open = function() {
	...
	if(currExt=="cbr" || currExt=="cbz") {
		...
		this.checkSystemDefaultHandler(this.openImgFile.bind(this));
	} else if(this.isInFilter(this.TEXTFILTER)) {
		if(currExt=="html" || currExt=="htm" || currExt=="php" || currExt=="asp") {
			this.openTxtFile();
		} else {
			this.checkSystemDefaultHandler(this.openTxtFile.bind(this));
		}
	} else if(currExt == "patch" || currExt == "diff") {
		this.checkSystemDefaultHandler(this.openPatchFile.bind(this));
	} else if(currExt == "ipk") {
		this.checkSystemDefaultHandler(this.openIpkFile.bind(this));
	} else { 
		this.palmOpenFileGeneric(this.checkForTxtFile.bind(this));
	}
};

ExplorerAssistant.prototype.checkSystemDefaultHandler = function(internalzCallback) {
	this.controller.serviceRequest("palm://com.palm.applicationManager", {
		method: "getHandlerForExtension",
		parameters: {
			extension:this.currItem.type
		},
		onSuccess: function(payload) {
			if(payload.appId == "ca.canucksoftware.internalz") {
				internalzCallback();
			} else {
				this.palmOpenFileSpecific(payload.appId, function() {
...
ExplorerAssistant.prototype.openImgFile = function() {
	var imgParams = {path:this.currItem.path, filemgr:this.filemgr, images:this.imgList};
...
ExplorerAssistant.prototype.openPatchFile = function() {
	var patchParams = {path:this.currItem.path, filemgr:this.filemgr};
	if(this.appAssist.settings.editor.newCard) {
		this.openSceneInNewCard('patcher', patchParams);
	} else {
		this.controller.stageController.pushScene('patcher', patchParams);
	}
};

ExplorerAssistant.prototype.openIpkFile = function() {
	var patchParams = {path:this.currItem.path, filemgr:this.filemgr};
	if(this.appAssist.settings.ipk.newCard) {
		this.openSceneInNewCard('ipkinstaller', patchParams);
	} else {
		this.controller.stageController.pushScene('ipkinstaller', patchParams);
	}
};
...
ExplorerAssistant.prototype.activate = function(event) {
	...
	if(Preferences.get("handlerSplash", {shown:false}).shown==false) {
		this.filemgr.exists(Mojo.appPath.substring(7) + "ran-setup", function(response) {
			if(response.exists==false) {
				this.controller.stageController.pushScene("handlers",
						{filemgr:this.filemgr});
			}/* else {
				Preferences.set("handlerSplash", {shown:true});
				this.checkForInternalz();
			}*/
		}.bind(this));
	}/* else {
		this.checkForInternalz();
...
ExplorerAssistant.prototype.checkForInternalz = function(event) {
	if(Preferences.get("thankyou", {shown:false}).shown==false) {
		this.filemgr.isPackageInstalled("ca.canuckcoding.internalz", function(response) {
			if(response.installed) {
				this.filemgr.exists(Mojo.appPath.substring(7) + "ran-thankyou", function(response2) {
					if(response2.exists==false) {
						this.controller.showAlertDialog({
							title: "",
				    		message: "<center style=\"margin-top:-10px\">"
									+ "<img src=\"images/thankyou.png\"/><br>"
									+ $L("Your purchase of Internalz from the App Catalog "
									+ "is deeply appreciated.") + "<br><br>"
									+ $L("How would you like to integrate it?")
									+ "</center>",
				    		choices:[
				        		{label:$L("Hide Internalz"), value:"hide1", type:"affirmative"},
								{label:$L("Hide Internalz Pro"), value:"hide2", type:"affirmative"},
				        		{label:$L("Do Nothing"), value:"show", type:"negative"},
								{label:$L("Ask Later"), value:"later", type:"dismiss"}, 
				    		],
							allowHTMLMessage: true,
							onChoose: function(value) {
								if(value=="hide1") {
									MsgBox(this, $L("The change will take effect after the next system "
											+ "restart."));
									this.filemgr.hideInternalz(function() {
										this.filemgr.createFile(Mojo.appPath.substring(7)
												+ "ran-thankyou");
									}.bind(this));
								} else if(value=="hide2") {
									MsgBox(this, $L("The change will take effect after the next system "
											+ "restart."));
									this.filemgr.hideInternalzPro(function() {
										this.filemgr.createFile(Mojo.appPath.substring(7)
												+ "ran-thankyou");
									}.bind(this));

								} else if(value=="show") {
									Preferences.set("thankyou", {shown:true});
									this.filemgr.createFile(Mojo.appPath.substring(7)
											+ "ran-thankyou");
								}
							}.bind(this),
...
ExplorerAssistant.prototype.scrollToTop = function(event) {
	var scroller = this.controller.getSceneScroller();
	scroller.mojo.scrollTo(undefined, 0, true);
};
ExplorerAssistant.prototype.scrollToBottom = function(event) {
	var scroller = this.controller.getSceneScroller();
	var newTop = scroller.mojo.scrollerSize().height - this.controller.sceneElement.scrollHeight;
	if (newTop > -3) {
		newTop = 0;
	}
	scroller.mojo.scrollTo(undefined, newTop, true);
};
