...
function validForPrivate(assistant) {
	...
	return (appid=="" || appid=="ca.canucksoftware.internalz");
}

function writePermissable(path, appid, callback) {
	var BASE1 = "/media/cryptofs/apps/";
	var BASE2 = "/media/internal/";
	var BASE3 = "/var/";
	
	if(appid = "ca.canucksoftware.internalz") {
		var cmd = new CommandLine("/usr/sbin/rootfs_open -w");
		cmd.run(function(response) {
			callback((response.code==0));
		});
	} else {
		if((path.startsWith(BASE1) && path.length>BASE1.length) ||
				(path.startsWith(BASE2) && path.length>BASE2.length) ||
				(path.startsWith(BASE3) && path.length>BASE3.length)) {
			callback(true);
		} else {
			callback(false);
		}
